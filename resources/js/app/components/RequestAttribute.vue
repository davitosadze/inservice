    ...
    <tr>
        ...
        <td>
            <input
                ...
                @input="(event) => trigger_price(event, item)"
            />
        ...
        <td>
            <input
                ...
                @input="(event) => trigger_price(event, item, true)"
            />
        </td>
        <td v-if="model.type == 'evaluation'">
            <input
                :readonly="model.type == 'evaluation'"
                @focus="focus"
                @blur="blur"
                v-model="item.pivot.evaluation_price"
                step="any"
                class="form-control"
                name="price"
                type="text"
        ...
        <td>
            <input
                ...
                @focus="focus"
                @blur="blur"
                v-model="item.pivot.service_price"
                @input="(event) => trigger_price(event, item, true)"
            />
        </td>
        <td v-if="model.type == 'evaluation'">
            <input
                :readonly="model.type == 'evaluation'"
                v-model="item.pivot.evaluation_service_price"
                step="any"
                @focus="focus"
                @blur="blur"
                class="form-control"
        ...
        <td v-if="model.type == 'evaluation'">
            <input
                ...
                step="any"
                readonly
                :value="item.pivot.evaluation_calc"
                @focus="focus"
                @blur="blur"
            />
        </td>
...
export default {
    ...
    created() {
        if (this.item.category)
            this.item.pivot.title = this.fullName(this.item);
    },
    data() {
        return {};
    },
        ...
        watch(
            ...
            (newxValue, prevValue) => {
                ...
                if (
                    newxValue &&
                    prevValue &&
                    props.model.type == "evaluation"
                ) {
                    props.keys.map((key) => {
                        if (
                            newxValue.pivot[key.replace("evaluation_", "")] !==
                            prevValue.pivot[key.replace("evaluation_", "")]
                        ) {
                            if (key == "evaluation_calc") {
                                props.item.pivot[key] = Util.numberFormat(
                                    (newxValue.pivot.evaluation_price +
                                        newxValue.pivot
                                            .evaluation_service_price) *
                                        newxValue.pivot.qty
                                );
                            } else {
                                props.item.pivot[key] = Util.itemReportValue(
                                    newxValue.pivot[
                                        key.replace("evaluation_", "")
                                    ],
                                    instance.parent.parent.proxy
                                        .initReporteValues
                                );
                            }
                        }
                    ...
                    if (newxValue.pivot.calc !== prevValue.pivot.calc) {
                        ...
                        props.item.pivot.evaluation_calc = Util.numberFormat(
                            (newxValue.pivot.evaluation_price +
                                newxValue.pivot.evaluation_service_price) *
                                newxValue.pivot.qty
                        );
                    }
                } else if (newxValue) {
