    ...
    <div class="card" :id="'card-' + index">
        <div class="card-header" :id="`heading${index}`">
            <h5 class="mb-0">
                ...
                <button
                    @click="(e) => remove(e, model.items[index] || index)"
        ...
        >
            <div class="card-body">
                <div class="form-group" v-for="(item, i) in tablist">
                    ...
                    <input
                        type="hidden"
                        :name="`item[${index}][${i + 1}][uuid]`"
                        :value="item.uuid"
                    />
                    <input
                        type="hidden"
                        :name="`item[${index}][${i + 1}][parent_uuid]`"
                        :value="item.parent_uuid"
                    />
                    <input
                        type="hidden"
                        :name="`item[${index}][${i + 1}][title]`"
                        :value="item.title"
                    />

                    <textarea
                        :name="`item[${index}][${i + 1}][value]`"
                        class="form-control"
                        >{{ item.value }}</textarea
                    >
                    ...
                    <update-media
                        ...
                        :index="index"
                        :each="i + 1"
                        server="/reports/uploads"
                        :media_server="`/reports/uploads2/${item.id}`"
                    ></update-media>
...
export default {
    ...
    computed: {
        ...
        titles() {
            return [
                "კვლევის საგნის დახასიათება:",
                "დაზიანების პირველადი აღწერილობა:",
                "დასკვნა:",
            ];
        },

        tablist() {
            return this.res.nested
                ? this.res.nested
            ...
            return Util.useSwall2(this.m).then((result) => {
                if (result.isConfirmed) {
                    ...
                    this.removeRequest(tabIndex ? tabIndex.id : null, () => {
                        let tabSplace = this.m.items.findIndex(
                            (i) => i.id == index.id
                        );
                        if (tabIndex) {
                            this.m.items.splice(tabSplace, 1);
                            document
                                .querySelector("#card-" + tabSplace)
                                .remove();
                        } else {
